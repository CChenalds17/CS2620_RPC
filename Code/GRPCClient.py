...
class UserClient:
    ...
    def query_accounts(self):
        ...
        query = ["All"] if not pattern else ["Like"] + [pattern] # Default is to list all accounts

        try:
            response = self.stub.GetUsers(chat_pb2.GetUsersRequest(query))
        except grpc.RpcError as e:
            messagebox.showerror("gRPC Error", str(e))
            return
    ...
    def logout(self):
        ...
        try:
            response = self.stub.ConfirmLogout(chat_pb2.ConfirmLogoutRequeset(username=self.username))
        except grpc.RpcError as e:
            messagebox.showerror("gRPC Error", str(e))
            return
        if response.status == chat_pb2.SUCCESS:
